...
function cardForItem(item, onDeleted) {
  // item: { id, url, thumbUrl, tinyUrl, createdAt, type }
  ...
  const img = createProgressiveImage(
    ...
    () => { ensureFilled(2); } // when an image settles, we might need more rows
  );
...
async function ensureFilled(maxLoops = 6) {
  let loops = 0;
  // Load while we have room or we're near the bottom (no scroll yet), but stop at max loops
...
function setupObserver() {
  ...
  io = new IntersectionObserver((entries) => {
    ...
    root: null,
    rootMargin: "1500px 0px", // prefetch well before hitting the bottom
    threshold: 0
  });
  io.observe(sentinel);
}

// Fallback: if IO doesn’t fire (or columns change), also use scroll/resize
let ticking = false;
...
/* ========== MutationObserver: when grid layout changes (breakpoints), try to fill ========== */
