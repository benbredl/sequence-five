...
(() => {
  ...
  // ----- Styles (matches original viewer look) -----
  ...
  function openPicker(imageId, backdropEl) {
    // Simple modal (inline styles so we don't rely on external CSS)
    const back = document.createElement("div");
    back.style.cssText =
      "position:fixed;inset:0;background:rgba(6,8,18,.58);backdrop-filter:blur(6px);display:flex;align-items:center;justify-content:center;z-index:10002";
    const sheet = document.createElement("div");
    sheet.style.cssText =
      "width:min(560px,92vw);background:rgba(12,15,32,.62);backdrop-filter: blur(12px) saturate(120%);border:1px solid rgba(71,80,124,.8);border-radius:18px;box-shadow:0 24px 80px rgba(2,6,23,.65), inset 0 1px 0 rgba(255,255,255,.06);padding:16px;color:#e6e9ff";
    sheet.innerHTML =
      "<h3 style='margin:0 0 10px 0;font-size:18px'>Add to Storyboard</h3>" +
      ...
      "<div style='margin-top:12px;display:flex;justify-content:flex-end'><button id='nbv-close' class='icon-btn' style='width:auto;height:auto;padding:7px 12px;border:1px solid #2f375a;border-radius:999px;background:linear-gradient(180deg,rgba(255,255,255,.07),rgba(255,255,255,.03));color:#c7ceed;font-weight:700'>Close</button></div>";
    ...
    fetch("/api/storyboards")
      ...
      .then((x) => {
        ...
        if (!x.ok) {
          list.innerHTML =
            "<div class='hint' style='padding:12px'>Failed to load</div>";
          return;
        }
        const arr = x.body.storyboards || [];
        if (!arr.length) {
          list.innerHTML =
            "<div class='hint' style='padding:12px'>You have no storyboards yet.</div>";
          return;
        }
        list.innerHTML = "";
        arr.forEach((sb) => {
          ...
          row.innerHTML =
            ...
            (sb.title || "") +
            "</div><div class='hint' style='font-size:12px;color:#a1a8be'>" +
            (sb.description || "") +
            "</div></div>";
          const add = document.createElement("button");
          add.className = "icon-btn";
          add.style.cssText =
            "width:auto;height:auto;padding:7px 12px;border:1px solid #2f375a;border-radius:999px;background:linear-gradient(180deg,rgba(255,255,255,.07),rgba(255,255,255,.03));color:#c7ceed;font-weight:700";
          add.textContent = "Add";
          add.onclick = () => {
            add.disabled = true;
            ...
            fetch("/api/storyboard/add", {
              ...
              .then((r) => r.json().then((j) => ({ ok: r.ok, body: j })))
              .then((y) => {
                if (!y.ok) throw new Error(y.body.error || "Failed");
  ...
  function open(urlLike, opts) {
    ...
    bAdd.title = opts.imageId
      ? "Add to Storyboard"
      : "Add to Storyboard (image not saved yet)";
    bAdd.innerHTML = SVG_ADD;
    if (opts.imageId) {
      bAdd.onclick = (e) => {
        e.stopPropagation();
        openPicker(opts.imageId, back); // blur while picker open
      };
    } else {
      bAdd.disabled = true;
    }
    ...
    if (opts.imageId) {
      bDel.onclick = async (e) => {
        e.stopPropagation();
        if (!confirm("Delete this image everywhere? This cannot be undone."))
          return;
        bDel.disabled = true;
        try {
  ...
  function attachHoverOverlay(cardEl, imgUrlLike, item, onDeleted) {
    ...
    aDown.onclick = (e) => {
      e.stopPropagation();
      if (imgUrl) forceDownload(imgUrl);
    };
    ...
    aAdd.onclick = (e) => {
      e.stopPropagation();
      if (item && item.id) openPicker(item.id);
    };
